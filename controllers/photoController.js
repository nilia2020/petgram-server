...
const photoControllers = {
    newPhoto: async (req, res) => {
        try {
            ...
            if (error.isEmpty()) {
                ...
                if (isSrcExist) {
                    return res.status(400).json({ error: "La foto ya esta en la base" });
                }
                ...
                const isCategoryIdExist = await Category.findById(req.body.categoryId);
    ...
    PhotosByCategoryId: async (req, res) => {
        const photos = await Photo.find({ categoryId: req.params.categoryId });
        res.status(200).json(photos);
    },
